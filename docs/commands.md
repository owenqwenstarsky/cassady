...
Cassady installs two equivalent CLI binaries: `cass` and `cassady`. This page uses `cass`, but the same options and subcommands apply to `cassady`. macOS/Linux release archives also include a `cassady-desktop` companion binary launched by `cass desktop`.
...
cass logout [OPTIONS]
cass setup [OPTIONS]
cass desktop [OPTIONS]
cass update [OPTIONS]
cass --resume [CHAT_ID]
```

Run `cass --help`, `cass check --help`, `cass login --help`, `cass logout --help`, `cass setup --help`, `cass desktop --help`, or `cass update --help` for the help generated by the current binary.
...
- `--foreground`: keep the desktop process attached to the terminal for debugging.

Use `cass --cwd PATH desktop` to choose the launch cwd/workspace root for the desktop app.
...
By default, Cassady selects the matching prebuilt archive for the current platform, downloads the archive and `.sha256` file, verifies SHA-256, stages `cass` and `cassady`, stages `cassady-desktop` when the archive contains it, and replaces same-directory binaries with rollback backups. If no prebuilt archive is available, it can build from the selected release source when you choose source mode.
